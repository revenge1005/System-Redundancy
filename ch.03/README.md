...
> 해당 구성과 같이 Switch3/4 사이에 LS3-4를 설정해두면 **Switch1~4가 모두 상호 연결돼 있어 루프가 형성**되어 있음을 알 수 있고, 이더넷에서는 이와 같은 **루프가 생기면 "브로드캐스트 스톰(브로드캐스트 패킷이 루프가 생긴 네트워크 내부를 무한히 돌아다니는 상태)"이 발생**한다.<br><br>
