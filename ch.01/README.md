...
+ NAT구성에서는 로드밸런서가 목적지 IP주소를 변경하에 메일서버가 받아들이는 패킷은 수신측 주소가 "메일서버", 송신측 주소가 "웹서버"됨

+ 따라서, 메일 서버가 반환하는 응답패킷의 송신측 "메일서버"는 동일한 서브넷인 IP 주소이므로 로드밸런서로 되돌아가지 않고 웹서버로 직접 송신됨
