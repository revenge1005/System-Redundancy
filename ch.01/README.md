...
- [x] ICMP의 Echo 요청을 보내서 응답이 돌아오는지를 체크 
...
- [x] TCP로 접속을 시험해서 접속할 수 있는지 여부를 체크 
...
- [x] 실제로 HTTP 요청 등을 보내서 정상적인 응답이 들어오는지 체크 
...
- [x] "서버 장애에 의한 서비스 중지"는 정상적으로 검출하기 위해서는 "서비스 감시"를 이용한다.
...
- [x] "라우터 장애에 의한 서비스 정지"를 검출하기 위해서는 ICMP 감시를 이용한다. 
