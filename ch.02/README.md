...
> IPVS(LVS)와 같은 Load Balancer는 L4 레벨에서 패킷을 전송하기만 할 뿐, 웹 서버가 Client으로부터의 요청에 직접 응답하는 구성이라는 점은 변함이 없다. 여기서 Load Balancer 대신 Reverse Proxy Server를 넣음으로써 보다 유연하게 부하를 분산할 수 있게 된다. <br><br>
