...
> IPVS(LVS)와 같은 Load Balancer는 L4 레벨에서 패킷을 전송하기만 할 뿐, Web Server가 Client으로부터의 요청에 직접 응답하는 구성이라는 점은 변함이 없다. 여기서 Load Balancer 대신 Reverse Proxy Server를 넣음으로써 보다 유연하게 부하를 분산할 수 있게 된다.

> Reverse Proxy는 Client로부터의 요청을 받아서 Web Server로 요청을 전송하면, Web Server는 요청을 받아서 처리를 하지만 응답은 Client로 보내지 않고 Reverse Proxy로 반환한다. 요청을 받은 Reverse Proxy는 그 응답을 Client로 반환한다.

<br>

## ⓐ 프록시(Proxy) 서버
> Client가 자신을 거쳐 다른 네트워크에 접속할 수 있도록 중간에서 대신 전달해주는 Server

<br>

#### 【 Forward Proxy 】
> Client에서 요청을 할 때 직접 요청하는 것이 아닌 Proxy Server를 거치는 방식 

<br>

+ **보안**

    - [x] Client는 Proxy를 통해서만 외부에 요청을 하기 때문에 특정 사이트에 직접적으로 접근하는 것을 방지할 수 있다.<br>

+ **캐싱**

    - [x] Client의 요청을 Proxy가 캐싱하여 동일한 요청이 들어온 경우 캐싱된 데이터를 전달함으로써 서버의 부하를 줄일 수 있다.<br>

+ **암호화**
    
    - [x] Client의 요청은 Proxy를 통해 암호화되어 다른 서버를 통과할 때 필요한 최소한의 정보만 갖게 되는데, 이는 Client의 IP를 숨길 수 있다.

<br>

#### 【 Reverse Proxy 】
> Server에서 Client로 직접 데이터를 전달하지 않고 프록시 서버를 거치는 방식 

<br>

+ **로드밸런싱**

    - [x] 여러 대의 Server에 트래픽을 분산시켜 서비스의 퍼포먼스와 신뢰성을 향상하기 위한 서비스 <br>

+ **보안**   

    - [x] Server의 IP를 노축시키지 않을 수 있기 때문에 Server에 대한 1차적인 공격을 막을 수 있다. <br>

+ **캐싱**

    - [x] Proxy Server에 캐싱되어 있는 데이터를 사용하여 Client에 대한 요청을 처리 <br>
...
+ **압축**
    - [x] Server의 응답을 Client로 반환하기 전에 압축하면 필요한 대여폭이 줄어들어 전송 속도 향상 
...
> Reverse Proxy가 있으면 HTTP 요청 내에서 URL 보고 최종적인 처리를 각기 다른 Server에 분배하는 제어가 가능해진다.

<br>

#### 【 시스템 전체의 메모리 사용효율 향상 】
> 정적 컨텐츠만 반환하는 Server에 비해 동적 컨텐츠를 반환하는 Server에서는 수 배의 메모리를 소비하는 것도 드물지 않다. **Server는 Client의 하나의 요청에 대해 하나의 프로세스/쓰레드를 할당해서 처리하는 방식을 취하고 있으며 정적 컨텐츠를 반환하는, 즉 파일에 쓰인 내용을 그대로 반환하기만 하면 될 경우도 동일한 방식으로 반환하게 된다.**

<br>

+ **동적으로 생성된 하나의 HTML 페이지 내에 이미지가 30개 정도 사용되고 있다고 가정할 때** <br><br>
    - [x] 이 페이지에 대한 요청은 첫 요청 한 번만 동적 컨텐츠를 요구하게 되고 첫 요청으로 HTML 동적으로 생성되며 이 HTML은 Client인 브라우저에 의해 다운로드 된다. <br><br>
    - [x] 브라우저는 HTML을 해석해서 필요한 이미지 파일이나 스크립트 파일을 Server에 요청한다. <br><br>
    - [x] 결과적으로 동적 요청 (1회) + 정적 요청 (30회)가 된다. <br><br>
+ **모두 AP Server에서 응답할 경우** <br><br>
    - [x] 모두 AP Server에서 응답할 경우, 단 1회 동적 요청을 처리할 뿐임에도 나머지 30회의 정적인 요청에 대해 응답할 때에도 대량의 메모리를 소비하게 된다. <br><br>
    - [x] 이는 이미지든 동적 컨텐츠든 똑같이 하나의 요청에 대해 하나의 프로세스/쓰레드로 응답할 필요가 있기 때문이다. <br><br>
+ **Server를 분할할 경우** <br><br>
    - [x] 정적 컨텐츠는 메모리 소비량이 적은 Web Server가 응답하고, 동적 컨텐츠만 애플레킹션으로 응답하는 형태의 구성이 가능해진다. <br><br>
    - [x] Reverse Proxy를 통해 정적 컨텐츠, 동적 컨텐츠에 대한 요청을 각각의 Server로 할당할 수 있다 (ex. URL의 내용을 보고 요청을 할당할 곳을 변경한다.) <br><br>
    - [x] 이때, Reverse Proxy 자신도 Web Server라는 특징을 살려서 (정적 컨텐츠를 반환하기 위해 Web Server를 별도로 준비하는 것이 아니라) 정적 컨탠츠는 Reverse Proxy 자신이 반환하는 형태의 구성이 일반적이다.

<br>

#### 【 Web Server가 응답하는 데이터의 버퍼링의 역할 】
+ **HTTP의 Keep-Alive** <br><br>
    - [x] 특정 Client가 한 번에 다수의 컨텐츠를 동일한 Web Server로부터 얻고자 할 경우, 다수의 HTTP 요청마다 Server와 접속하고 끊고를 반복하는 것은 비효율적이다. <br><br>
    - [x] 최초 요청 시 연결된 Server와의 접속을 해당 요청이 종료한 후에도 접속을 끊지 않고 유지한 채로 이어지는 요청에 해당 접속을 계속 사용함으로써, 하나의 접속으로 다수의 요청을 처리할 수 있도록 실현한 것이 Keep-Alive이다. <br><br>
    - [x] Kepp-Alive는 한번 연결된 접속을 당분간 유지하는 특성상, Web Server에 부하를 야기한다 <br><br>
    - [x] 구체적으로는 특정 Client로부터 요청을 받은 프로세스/쓰레드는 그 시점으로부터 일정 시간 동안 해당 Client로의 응답을 위해서 점유되는 것을 들 수 있다. <br><br>
+ **메모리 소비와 Keep-Alive ON/OFF** <br><br>
    - [x] 하나의 프로세스당 메모리 소비량이 많은 AP 서버에서는 하나의 호스트 내에 실행될 수 있는 최대 프로세스 수는 50 ~ 100개 정도다.  <br><br>
    - [x] 이때 **리버스 프록시 없이 Keep-Alive를 설정한 경우 50~100개의 프로세스 중 다수가 Keep-Alive의 접속 유지를 위해 소비되고 Keep-Alive를 OFF로 설정하면 Client에서 볼 때의 체감속도가 저하**된다. <br><br>
    ...
    - [x] 그리고 **Client와 리버스 프록시 사이에만 "Keep-Alive ON"으로 하고, 리버스 프록시와 AP 서버 사이는 Keep-Alive OFF로 한다.** <br><br>
    - [x] 이렇게 하면 AP 서버 측은 프로세스 수가 적더라도 하나의 요청이 종료되면 곧바로 그 후에 다른 요청에 응답할 수 있으며 전체적으로 동시에 다룰 수 있는 Client의 수는 많아지고 또한 전송량도 향상된다. <br><br>
+ **아파치 모듈을 이용한 처리의 제어** <br><br>
    - [x] 리버스 프록시로 아파치를 선택한 경우, 해당 리버스 프록시에 아파치 모듈을 내장해서 HTTP 요청의 전/후처리로 임의의 프로그램을 실행 시킬 수가 있다.  <br><br>
        - **mod_deflate :** 컨텐츠를 gzip 압축하는 아파치 모듈, AP 서버로부터 수신한 HTTP 응답을 Client에 압축해서 보낼 수가 있다. <br><br>
...
#### 【 캐시를 적용하기 좋은 데이터 】
> Server에 데이터를 요청하면, 해당 Server가 응답을 반환할 때까지 페이지가 로드되지 않는다 즉, 페이지 로딩에 필요한 정적 리소스를 캐싱하여 사용하게 되면 요청을 보내는 네트워크 요청 횟수를 줄일 수 있을 뿐만 아니라 Server 응답을 기다려야 할 필요도 없기 때문에 사용자에게 보다 빠르게 화면을 보여줄 수 있다.
...
## ⓑ Squid 캐시서버
+ Squid는 HTTP, HTTPS, FTP 등에서 이용되는 오픈소스 캐시 Proxy Server
...
+ 서버의 메모리(in-memory)에 저장하는 방식으로, 반환 속도가 빠르고 서버의 성능을 향상하는 것이 목적인 캐시에 최적화 되어 있다.
...
> MySQL 리플리케이션 기능으로 지원되고 있는 것은, 한 대의 마스터와 여러 슬레이브로 이루어진 구성이며, 슬레이브는 여러 대 존재할 수 있으므로 SELECT문 등의 참조 쿼리를 여러 슬레이브로 분산해서 성능 향상을 꾀할 수 있도록 구성할 수도 있다.
    ...
    - [x] **마스터 :** Client로부터 갱신과 참조 두 가지 쿼리를 받아들이는 서버 <br><br>
    - [x] **슬레이브 :** Client로부터 갱신 쿼리는 받아들이지 않으면서 데이터 갱신은 마스터와 연계를 통해서만 수행하는 역할을 하는 서버
...
> 슬레이브에서는 리플리케이션을 위해 두 개의 쓰레드가 동작하고 있으며, 이유는 리플리케이션의 지연을 줄이기 위해서이다.
...
- [x] 하지만 Real Server와 Client가 같은 네트워크에 존재할 때는, 로드밸런서를 도입할 필요없이 직접 Client로 패킷이 전송되어, 결과적으로 VIP 앞으로 전송된 패킷의 응답이 VIP와는 다른 곳(Real Server의 주소)으로부터 반환되어 오는 것처럼 보이는 것이다. <br><br>