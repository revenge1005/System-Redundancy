...
+ **대량의 접속되어 오더라도 병목이 되지 않을 정도로 빨라야 함**
...
> 접속이 집중될 때라는 것은 **"수많은 사용자가 동일한 데이터를 요청하고 있을 때"**이므로, 동일한 데이터를 반복해서 읽어내는 형태의 접속 패턴이 된다 이러한 데이터는 거의 스토리지 서버의 메모리에 캐싱되며, 경량 웹 서버(lightppd, nginx 등)은 메모리에 캐싱되어 있는 데이터를 오로지 전송만 하면 되므로 스토리지 서버의 디스크 I/O에는 거의 부하가 걸리지 않는다.


+ "WS"라는 서버가 스토리지 서버로, 파일 쓰기에는 NFS를 사용하지만 모든 서버를 마운트할 필요는 없다 파일 업로드하는 서버만 마운트되어 있으면 충분하다. 그림의 "마스터 서버"가 해당한다.

+ 다른 서버는 NFS를 이용하지 않고, HTTP를 경유하여 파일을 취득한다.