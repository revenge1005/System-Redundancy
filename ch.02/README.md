...
+ Web Server가 스토리지 서버를 NFS 마운트해서 이용하는 경우라고 가정하고 내용을 진행한다. man nfs의 내용은 다음과 같다.
    ...
    - [x] **hard :** NFS 파일 작업에서 주 타임아웃이 걸리면 콘솔상에 "server not responding"이라 출력하고 **끊임없이 재시도한다.**

    - [x] **intr :** NFS 파일 작업에서 주 타임아웃이 걸리고 NFS 접속이 hard 마운트된 상태라면 시그널에 의해 파일 작업을 중지하도록 허용하고, 중단된 경우에는 호출한 프로그램에 EINTR를 반환한다. **(기본값은 파일 작업을 인터럽트하지 않는다.)**
