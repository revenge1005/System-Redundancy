...
+ **보안**
    ...
    - [x] Client는 Forward Proxy를 통해서만 외부에 요청을 하기 때문에 특정 사이트에 직접적으로 접근하는 것을 방지할 수 있다.<br><br>

+ **캐싱**

    - [x] Client의 요청을 Forward Proxy가 캐싱하여 동일한 요청이 들어온 경우 캐싱된 데이터를 전달함으로써 서버의 부하를 줄일 수 있다.<br><br>
