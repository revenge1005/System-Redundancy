...
+ Squid는 HTTP, HTTPS, FTP 등에서 이용되는 오픈소스 캐시 Proxy Server <br>
...
## ⓐ 스토리지 서버의 필요성
> 부하분산 환경에서는 **여러 대의 Web Server에 동일한 파일을 저장해야 하지만, 파일의 수나 크기가 방대해지면 다음과 같은 문제가 발생**한다.

<br>

+ **모든 Web Server로 배치(Deploy)시키는는 데에는 시간이 걸린다.**

+ **모든 Web Server에 대용량의 하드디스크를 탑재하야 한다.**

+ **모든 Web Server의 파일이 정합성(데이터가 서로 모순 없이 일관되게 일치해야 함)을 갖는지 검증하기가 곤란하다.**

+ **Web Server를 신규로 도입하기가 곤란하다(파일 복사에 시간이 걸린다).**
...
+ **만일 데이터가 소실되면 복구하는데 엄청난 시간과 노력이 소요된다.**
...
+ Web Server가 스토리지 서버를 NFS 마운트해서 이용하는 경우 (man nfs의 내용은 다음과 같다.)

    - **soft** 
    
        - [x] NFS 파일 작업에서 주 타임아웃이 걸리면 호출한 프로그램에 I/O에러를 보고한다. **(기본값은 끊임없이 파일 작업을 재시도한다.)**

    - [x] **hard**

        - [x] NFS 파일 작업에서 주 타임아웃이 걸리면 콘솔상에 "server not responding"이라 출력하고 **(끊임없이 재시도한다.)**

    - [x] **intr** 
    
        - [x] NFS 파일 작업에서 주 타임아웃이 걸리고 NFS 접속이 hard 마운트된 상태라면 시그널에 의해 파일 작업을 중지하도록 허용하고, 중단된 경우에는 호출한 프로그램에 EINTR를 반환한다. **(기본값은 파일 작업을 인터럽트하지 않는다.)**

<br>

+ 즉, **스토리지 서버가 정지하고 있는 동안 Web Server는 NFS로 파일 작업을 끊임없이 재시도하는데, 그 결과 Web Server는 파일 작업 대기 프로세스로 가득 차서 다른 페이지도 조회할 수 없는 상태(서비스 정지)에 빠져버리게 된다.**

<br>

+ 마운트 옵션으로 soft와 intr을 지정함으로써 어느 정도 개선은 할 수 있지만, **NFS는 운영체제의 기능으로서 구현되어 있으므로 Web 애플리케이션에서 타임아웃 시간을 조정하거나 파일 작업을 중단할 수는 없다 따라서 파일 작업이 타임아웃되기를 기다리는 동안 Web Server의 프로세스가 가득 차게 되면 서비스는 정지해버린다.**